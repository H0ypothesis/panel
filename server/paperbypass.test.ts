...
test("Paperbypass streams Luna Pro with Messages routing and server-side Bearer auth", async (t) => {
  const modelId = "openai/gpt-5.6-luna-pro";
  ...
  process.env.PAPERBYPASS_MODEL = modelId;
  ...
  assert.deepEqual(updates, ["Hello", "Hello from Luna"]);
  assert.equal(result.response, "Hello from Luna");
...
test(
  "Paperbypass preserves streamed tool calls and results through the Pi agent loop",
  { timeout: 10000 },
  async (t) => {
    const modelId = "openai/gpt-5.6-luna-pro";
    const names = [
