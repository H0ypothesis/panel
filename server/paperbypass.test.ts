...
for (const { modelId, reply, contextWindow } of [
  ...
  test(`Paperbypass streams ${modelId} with Messages routing and server-side Bearer auth`, async (t) => {
    ...
    process.env.PAPERBYPASS_API_KEY = dummyKey;
    process.env.PAPERBYPASS_BASE_URL = `${origin}/api/`;
    delete process.env.PAPERBYPASS_MODEL;
    delete process.env.ATRIA_API_KEY;
    process.env.PANEL_DEFAULT_MODEL = configId;
    ...
    assert.equal(new URL(request.url!, origin).pathname, "/api/v1/messages");
  ...
  test(
    `Paperbypass preserves ${modelId} streamed tool calls and results through the Pi agent loop`,
    { timeout: 10000 },
    async (t) => {
      const names = [
        "PAPERBYPASS_API_KEY",
      ...
      const server = createServer(async (request, response) => {
        ...
        if (requests.length <= 2) {
          ...
          send({
            ...
            delta: {
              type: "text_delta",
              text: "Read complete; command denied.",
            },
          });
        }
      ...
      process.env.PAPERBYPASS_API_KEY = "mock-local-tools-key";
      process.env.PAPERBYPASS_BASE_URL = `${origin}/api/`;
      delete process.env.PAPERBYPASS_MODEL;
      const registry = createModels();
      registry.setProvider(paperbypassProvider());
      const model = registry.getModel("paperbypass", modelId);
      assert.ok(model);
      ...
      for (const request of requests) assert.equal(request.model, modelId);
...
test("Paperbypass retains Atria alongside a custom model without duplicates or credentials", (t) => {
  const names = ["PAPERBYPASS_API_KEY", "PAPERBYPASS_MODEL"] as const;
  ...
  for (const key of [undefined, "   "]) {
    ...
    for (const id of ["gateway/custom-model", "Atria-Dawn-Preview"]) {
      ...
      assert.ok(models.some((model) => model.id === `paperbypass/${id}`));
      assert.equal(
        models.filter((model) => model.id === "paperbypass/Atria-Dawn-Preview")
          .length,
        1,
      );
      assert.ok(
